...
## [unreleased]
- FIX: A DataFrame the profile asked to store as parquet, which pyarrow then could not convert, was stored with the default encoding and nothing was said. The dependency lookup and the conversion shared one `except Exception`, so an install present but broken against its numpy --- or a dtype, index or duplicate column name pyarrow will not take --- was indistinguishable from the extra not being installed at all. Someone who chose parquet precisely so that other tools could read the data found out much later, if ever. pyarrow being absent still falls back silently, which is the documented behaviour and the only case worth no words; a conversion that fails now warns with `FrameEncodingFallbackWarning`, naming the frame's shape and the underlying error. Either way the save still succeeds --- a save must not fail because an optional codec has a limitation
