...
- Blob compression is decided per value by sampling the data, because blanket compression measured wrong in both directions — about 8x smaller in 10ms on a rounded price series, and 4% for several seconds on random floats, both float64 arrays. `"auto"` compresses a 256 KiB sample, extrapolates, and stores raw unless the saving exceeds 10%. Override with `compression="none"`, `"zlib:N"` or `"zstd:N"` on a `SerializationProfile`
